    ...
    public partial class MemberElement
    {
        public static partial class EmitHelper
        {
            ...
            public static ConstructorHandler MyCreateInstantiateObjectHandler(Type type)
            {
                ...
                if (type.IsValueType && !Helpers.Types.IsNullableType(type))
                {
            ...
            internal static Delegate CreateNativeGetHandler(MemberElement root)
            {
                ...
                if (Helpers.Types.IsNullableType(root._lastNullableReturnType) && !Helpers.Types.IsNullableType(returnType))
                {
                ...
                if (isLblNullUsed)
                {
                    if (Helpers.Types.IsNullableType(root._lastNullableReturnType))
                    {
        ...
        static Type EmitMember(ILGenerator il, MemberElement member, Label lblNull, ref bool isLblNullUsed, bool loadByAddress)
        {
            if (Helpers.Types.IsNullableType(member._instanceType))
            {
                ...
                Helpers.Emit.EmitCall(il, miValue);
                if (member.IsField)
                {// Value is Field
                    EmitField(il, member);
                }
                ...
                {
                    LocalBuilder var1 = il.DeclareLocal(Helpers.Types.GetNotNullableType(member._instanceType));
                    il.Emit(OpCodes.Stloc, var1);
                    il.Emit(OpCodes.Ldloca, var1);
                    Helpers.Emit.EmitCall(il, (MethodInfo)member._memberInfo);
                }
            }
            else if (member._instanceType.IsValueType)
            {// Value type
                if (member.IsField)
                {
                    EmitField(il, member);
        ...
        static bool IsFieldAddress(MemberElement member)
        {
            ...
            return (returnType.IsValueType && !Helpers.Types.IsNullableType(returnType) && member._child != null);
        }
    }
