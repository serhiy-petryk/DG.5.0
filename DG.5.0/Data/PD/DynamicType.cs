...
namespace Data.PD
{
    public static class DynamicType
    {
        ...
        public static Type GetDynamicType_FIELDS(IList<string> propertyNames, IList<Type> propertyTypes, Dictionary<string, Attribute[]> customAttributes, string[] primaryKey)
        {
            ...
            TypeBuilder tb = _moduleBuilder.DefineType("DynamicType_" + Helpers.Tips.GetUniqueNumber().ToString(), TypeAttributes.Public);
        ...
        public static Type GetDynamicType_PROPERTIES(IList<string> propertyNames, IList<Type> propertyTypes, Dictionary<string, Attribute[]> customAttributes, string[] primaryKey)
        {
            ...
            // Add new type to module
            var tb = _moduleBuilder.DefineType("DynamicType_" + Helpers.Tips.GetUniqueNumber(), TypeAttributes.Public);
            Dictionary<string, FieldBuilder> fields = new Dictionary<string, FieldBuilder>(StringComparer.OrdinalIgnoreCase);
        ...
        {
            var nestedPropertyName = parentPropertyName + Settings.MDelimiter + propertyName;
            propertyType = Helpers.Types.GetNullableType(propertyType);
            var isPropertyValueType = propertyType.IsValueType;

            var aa1 = propertyName.Split(new string[] { Settings.MDelimiter }, StringSplitOptions.None);
            if (aa1.Length > 5)
                return null;
            ...
            if (isPropertyValueType)
                il.Emit(OpCodes.Newobj, propertyType.GetConstructor(new[] { Helpers.Types.GetNotNullableType(propertyType)}));
        ...
        private static void BuildMethod_CompareTo(TypeBuilder tb, FieldBuilder[] fields)
        {
            ...
            {
                il.Emit(OpCodes.Ldarg_0);
                Helpers.Emit.EmitLoadField(il, fields[0]);
                il.Emit(OpCodes.Ldarg_1);
                il.Emit(OpCodes.Castclass, tb);
                il.Emit(OpCodes.Ldfld, fields[0]);
                Helpers.Emit.EmitCall(il, fields[0].FieldType.GetMethod("CompareTo", BindingFlags.Public | BindingFlags.Instance, null, new Type[] { fields[0].FieldType }, null));
                il.Emit(OpCodes.Ret);
            }
            else
            {
                ...
                {
                    il.Emit(OpCodes.Ldarg_0);
                    Helpers.Emit.EmitLoadField(il, fields[i]);
                    il.Emit(OpCodes.Ldloc, lbObject);
                    il.Emit(OpCodes.Ldfld, fields[i]);
                    Helpers.Emit.EmitCall(il, fields[i].FieldType.GetMethod("CompareTo", BindingFlags.Public | BindingFlags.Instance, null, new Type[] { fields[i].FieldType }, null));
        ...
        private static void BuildMethod_Equals(TypeBuilder tb, FieldBuilder[] fields)
        {
          ...
          baseMethod.CallingConvention, baseMethod.ReturnType, new Type[] { typeof(object) });
            ...
            {
                il.Emit(OpCodes.Ldarg_0);
                Helpers.Emit.EmitLoadField(il, fields[i]);// Get field value of first object
                il.Emit(OpCodes.Ldloc, lbObject);
                il.Emit(OpCodes.Ldfld, fields[i]);// Get field value of second object
                Helpers.Emit.EmitCall(il, fields[i].FieldType.GetMethod("Equals", BindingFlags.Public | BindingFlags.Instance, null, new Type[] { fields[i].FieldType }, null));
        ...
        private static void BuildMethod_GetHashCode(TypeBuilder tb, FieldBuilder[] fields)
        {
          ...
          baseMethod.CallingConvention, baseMethod.ReturnType, new Type[0]);
            ...
            for (int i = 0; i < fields.Length; i++)
            {
                ...
                Helpers.Emit.EmitCall(il, miGetHashcode);
        ...
        private static void BuildMethod_ToString(TypeBuilder tb, FieldBuilder[] fields)
        {
          ...
          baseMethod.CallingConvention, baseMethod.ReturnType, new Type[0]);
            ...
            if (fields.Length == 1)
            {
                ...
                Helpers.Emit.EmitCall(il, miToString);
                il.Emit(OpCodes.Ret);
            }
            else
            {
                ...
                il.Emit(OpCodes.Ldloc, lbStrings);
                Helpers.Emit.EmitCall(il, typeof(string).GetMethod("Concat", BindingFlags.Public | BindingFlags.Static, null, new Type[] { typeof(string[]) }, null));
                il.Emit(OpCodes.Ret);
            }
