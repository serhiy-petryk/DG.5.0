...
namespace Data.DB
{
    public class DbSchemaTable
    {
        ...
        {
            var key = DbHelper.Command_GetKey(cmd);
            lock (SchemaTables)
            {
        ...
        {// must be command with parameters (for SqlClient)
            ...
            using (DataTable dt = DbHelper.GetSchemaTable(cmd))
            {
            ...
            {
                var columnDescriptions = Helpers.DbMetaData.GetColumnDescriptions(cmd.Connection, tableName);
