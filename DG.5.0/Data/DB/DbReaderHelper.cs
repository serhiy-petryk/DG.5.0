...
namespace Data.DB
{
    public static class DbReaderHelper
    {
        ...
        static Func<DbDataReader, T> GetDelegate_ObjectFromDataReader<T>(DbColumnMapElement[] columnMap)
        {
            ...
            lock (_delegates)
            {
                ...
                foreach (DbColumnMapElement e in columnMap)
                {
                    ...
                    Type notNullableObjectType = Helpers.Types.GetNotNullableType(e.ItemDataType);
                    ...
                    if (e.CanBeNull)
                    {
                        ...
                        Helpers.Emit.EmitCall(il, mi); // is null ?
                    ...
                    Helpers.Emit.EmitCall(il, miReadFromReader);
                    //            il.Emit(OpCodes.Callvirt, miReadFromReader);
                    if (flagGetValue && dbType.IsValueType)
                    {// Need to unbox value type if no direct value exists (after (object)GetValue call)
                    ...
                    // ===========  Conversion
                    if (notNullableObjectType != dbType)
                    {// Need to convert
                        if (objectTypeConverter == null)
                        {
                            Helpers.Emit.EmitCall(il, Helpers.Types.GetConvertMethodInfo(dbType, notNullableObjectType));
                        ...
                        else
                        {
                            if (objectTypeConverter != null)
                            {
                                ...
                                MethodInfo miConvert = typeof(DbReaderHelper).GetMethod("ConvertToObject", BindingFlags.Static | BindingFlags.Public);
                                Helpers.Emit.EmitCall(il, miConvert.MakeGenericMethod(notNullableObjectType));
                    ...
                    if (Helpers.Types.IsNullableType(e.ItemDataType))
                    {
                        if (e.CanBeNull)
                        {
                        ...
                        {// Value can not be null (for primary keys and so on)
                            ConstructorInfo ciNullableGeneric = e.ItemDataType.GetConstructor(new Type[] { notNullableObjectType });
                            il.Emit(OpCodes.Newobj, ciNullableGeneric);
                        }
                    }
                    ...
                    {
                        Helpers.Emit.EmitCall(il, ((PropertyInfo)((PD.IMemberDescriptor)e.MemberDescriptor).ReflectedMemberInfo).GetSetMethod());
        ...
        static TypeClass GetTypeClass(Type type)
        {
            ...
            if (type.IsValueType && Helpers.Types.IsConvertible(type)) return TypeClass.NumberOrBoolean;
